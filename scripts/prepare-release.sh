...
request against master. Merging the pull request triggers the publish
workflow (.github/workflows/release.yml), which packages the extension,
publishes it to the Marketplace, pushes the `vVERSION` tag, and creates
the GitHub release.
...
Options:
  ...
  --pin REVISION    Update the pinned JETLS release in JETLS_VERSION.json
                    to REVISION (YYYY-MM-DD) before releasing
  --remote REMOTE   Remote to use (default: origin)

Examples:
  ./scripts/prepare-release.sh --no-push 2026.8.23
  ./scripts/prepare-release.sh --pin 2026-08-23 2026.8.23
EOF
}
...
if ! gh api "repos/$JETLS_REPOSITORY/git/ref/tags/$REVISION" >/dev/null; then
    echo "Error: pinned JETLS release tag $REVISION not found in $JETLS_REPOSITORY."
    exit 1
fi
scripts/check-julia-bounds.sh
...
sed -i.bak -E \
    "s|^([[:space:]]*placeholder: )\"v20[0-9]{2}\.[0-9]+\.[0-9]+\"$|\1\"v$VERSION\"|" \
    "$BUG_REPORT"
rm "$BUG_REPORT.bak"
if ! grep -Fq "placeholder: \"v$VERSION\"" "$BUG_REPORT"; then
    echo "Error: failed to update the version placeholder in $BUG_REPORT."
    exit 1
fi
...
Merging this PR publishes the extension to the Marketplace, pushes the \`$TAG\` tag, and creates the GitHub release."
